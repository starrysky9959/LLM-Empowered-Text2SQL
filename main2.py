...
import util
from vllm import LLM, Sa
print(torch.version.cuda)
model_path = "../LLaMA-Factory/041504_output_step_2_galore"
...
# with open("0322_step_1_output.json", "r") as input_file:
#     step_1_set = json.load(input_file)
    # dataset = json.load(input_file)
with open("cot_dataset/dev/step_2.json", "r") as input_file:
# with open("valid_dataset.json", "r") as input_file:
    step_2_set = json.load(input_file)
for i in range(len(step_2_set)):
    ...
    instruction = step_2_set[i]["instruction"]
...
with open("041601_galore_dev.json", "w") as output:
