...
def sql2skeleton(sql: str, db_schema:dict)->str:
...
if __name__ =="__main__":
    sql = "SELECT s.name, COUNT(sc.concert_id) as num_concerts FROM singer s LEFT JOIN singer_in_concert sc ON s.singer_id = sc.singer_id GROUP BY s.name;"
    normal = sql_normalization(sql)
    # sk = sql2skeleton(sql)
    parsed_sql = Parser(normal)
    print(normal)
    print(parsed_sql.generalize)
